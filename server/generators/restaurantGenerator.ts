/**
 * Restaurant & Cafe Domain Architectural Adviser & Dynamic Generator
...
export function generateRestaurantSite(prompt: string, analysis?: any): string {
  const guidance = getDomainGuidance('restaurant_cafe', prompt);

  const enrichedAnalysis = {
    ...analysis,
    siteType: analysis?.siteType || guidance.domainName,
    suggestedTitle: analysis?.suggestedTitle || (prompt.length > 25 ? prompt.substring(0, 25) : prompt) || 'مطعم وضيافة',
