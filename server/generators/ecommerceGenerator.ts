/**
 * E-commerce Domain Architectural Adviser & Dynamic Generator
...
export function generateEcommerceSite(prompt: string, analysis?: any): string {
  const guidance = getDomainGuidance('ecommerce_store', prompt);

  const enrichedAnalysis = {
    ...analysis,
    siteType: analysis?.siteType || guidance.domainName,
    suggestedTitle: analysis?.suggestedTitle || (prompt.length > 25 ? prompt.substring(0, 25) : prompt) || 'متجر إلكتروني',
